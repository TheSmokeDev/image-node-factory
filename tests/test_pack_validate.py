...
_SCRIPT = Path(__file__).resolve().parents[1] / "marketplace/image-node-factory" / "scripts" / "pack-validate.py"
...
def test_more_than_ten_concepts_is_rejected(tmp_path: Path) -> None:
    concepts = [{"baked_prompt": "Subject: x.", "overlay_prompt": "Subject: x.",
                 "copy": {}} for _ in range(11)]
    v = _violations(tmp_path, pack={"concepts": concepts, "prompt_count": 11})
    assert any("exceeds the cap of 10" in x for x in v)
