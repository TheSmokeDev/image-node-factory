...
_SCRIPT = Path(__file__).resolve().parents[1] / "marketplace/image-node-factory" / "scripts" / "style-corpus.py"
_WORKFLOW = Path(__file__).resolve().parents[1] / "marketplace/image-node-factory" / "image-node-factory.yaml"
...
def _write_corpus(root: Path, *, pin: str = _TEST_PIN, cases: dict | None = None) -> Path:
    ...
    sc.PINNED_FILES[pin] = {name: (remote, sc._sha256((d/name).read_bytes())) for name,(remote,_) in sc.CORPUS_FILES.items()}
    return d
...
def test_prime_installs_atomically(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sc.PINNED_FILES, _TEST_PIN, {name:(remote,sc._sha256(blobs[remote])) for name,(remote,_) in sc.CORPUS_FILES.items()})
...
def test_select_preserves_complete_prompt(corpus):
    g = sc.select(corpus, category="Illustration & Art", k=1)
    (ex,) = g.exemplars
    assert ex.truncated is False
    assert len(ex.prompt) == 1801
    assert ex.prompt.endswith("y" * 900), "the full example must survive"
